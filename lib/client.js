window.__ModuleLoader__.load({ id: '@qq410350809/dsh-model-enhance', factory: (require) => { var module = { exports: {} }; var exports = module.exports;
"use strict";
...
var PROVIDER_LABEL_NS = "dsh-model-enhance";
...
function providerLabelsOf(section) {
  const labels = {};
  const providerMap = section?.providers;
  if (isPlainObject(providerMap)) {
    for (const [name, value] of Object.entries(providerMap)) {
      ...
      for (const entry of profile.models) {
        ...
        if (id !== void 0) labels[id] = display_name;
        if (modelName !== void 0) labels[modelName] = display_name;
      }
    }
  }
  return labels;
...
function ModelEnhanceSection({ api, prefs, t, onInvalidate }) {
  ...
  const toggleProviderLabel = async () => {
...
function installProviderLabelBadges(getLabels) {
  const BADGE_ATTR = "data-dsh-provider";
  const CSS_ID = "dsh-model-enhance-provider-label-css";
  const providerFor = (text) => {
    const trimmed = text.trim();
    if (trimmed.length === 0) return void 0;
    const labels = getLabels();
    return Object.prototype.hasOwnProperty.call(labels, trimmed) ? labels[trimmed] : void 0;
  };
  const ensureCss = () => {
...
function apply(ctx) {
  ...
  const prefs = ctx.settingsScope.bind({ namespace: PROVIDER_LABEL_NS });
  let labels = {};
  const refreshLabels = async () => {
    try {
      ...
      labels = providerLabelsOf(view.user ?? view.value);
    } catch {
    }
  };
